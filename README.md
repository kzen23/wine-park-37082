...
## フロント作業
ビュー作成は、  
マークアップ言語を学習する時に良く触れていたという感じでした  
ので、今回、一から作成する際、  
とても困難を要しました。  

頭を抱えながら、検索や資料に  
目を通しながら、取り組み続けた結果、ある程度の形に表現する  
ことが出来る様になりました。  

この体験から自分が頭や手を  
動かしたものが、身に付くのかもしれないと思うようになりました。  

## 画像データ
学習時、heic画像が扱えなかったので、使えないだろうかと検索をしたところ、
実装の情報を見つけることが出来たので、今回は、それに倣って実装を行いました。 

## 年齢認証
今回は、お酒(ワイン)を取り扱うアプリケーションなので、  
20歳未満のユーザーが万が一に  
訪れた場合、新規登録できて  
しまうことは望ましくないと思いました。  
...
やってみると、自分はこう思っていたけれど、この作業には、  
その前にこれが必要だった、などとズレを感じることが多く、  
良く戸惑いました。おかげさまで設計段階の重要性を感じました。  
...
検索をして、情報を選択したり、複数の情報を組み合わせて、実装しました。  

情報と自身の環境が異なっていたので、自身の環境に適応させるには、どうしたらいいのだろう？と  
悩みましたが、そのおかげで以前より理解力が増したように思います。

## 結合テスト
結合テストは、単体テストに比べると経験が浅いので苦労しましたが、
exampleの洗い出しなど考えて吐き出せるようになりました。  

コメント機能の編集の結合テストは、コメントの紐付けの仕方が  
分からなく、とても苦労しました。  

この苦労をきっかけに問題解決のために外部の方と縁をし、チャットを通じてコミュニケーションが  
ちゃんと取れていた事、また現場で行われているという簡潔な記述をご教授頂くことが出来、成長を感じる事が出来ました。  
